...
const subtexts = [
  ...
  "You've successfully completed your basic sign-up with CARGURU!",
  "One platform. Endless opportunities. Let’s drive success together.",
  "Pre-Approval, Minus the Pain. You’re almost there!",
  "Simplifying Car Financing: Pre-Approval Made Easy",
  "Let’s Fast-Track Your Car Loan!",
  "We’ll let you know once we’ve done all the necessary verification."
];
...
function showStep(index) {
  ...
  // 🔥 Lines active when right-side circle (even index) is active
  const lines = document.querySelectorAll(".lines");
  lines.forEach((line, i) => {
    line.classList.toggle("active", index >= (i * 2 + 1));
  });
...
function change(){
    document.querySelector(".stepper").classList.toggle("d-none");
}
...
function populatePersonalSummary() {
  ...
  // --- Existing summary bindings ---
  document.querySelectorAll("#summaryName").forEach((el) => (el.textContent = name));
  document.querySelectorAll("#summaryIC").forEach((el) => (el.textContent = ic));
  document.querySelectorAll("#summaryMobile").forEach((el) => (el.textContent = mobile));
  document.querySelectorAll("#summaryEmail").forEach((el) => (el.textContent = email));
  document.querySelectorAll("#summaryAddress").forEach((el) => (el.textContent = fullAddress || ""));

  // Driving License text
  document.querySelectorAll(".summaryLicense").forEach((el) => {
    if (license === "Yes") {
      el.textContent = "Yes, I have a Driving Licence";
    } else if (license === "No") {
      el.textContent = "No, I don’t have a Driving Licence";
    } else {
      el.textContent = "";
    }
  });
  ...
  document.querySelectorAll(".summaryLicenseIcon").forEach((icon) => {
    icon.classList.remove("fa-check-square", "fa-times-circle", "fa-question-circle", "has-license", "no-license", "text-muted");

    if (license === "Yes") {
      icon.classList.add("fas", "fa-check-square", "has-license");
    } else if (license === "No") {
      icon.classList.add("fas", "fa-times-circle", "no-license");
    } else {
      icon.classList.add("fas", "fa-question-circle", "text-muted");
    }
  });
  ...
  if (profilePreview && profilePreview.src) {
    document.querySelectorAll("#summaryProfileImage").forEach((img) => {
      img.src = profilePreview.src;  // Copy uploaded pic into summary
...
document.getElementById("nextBtn").addEventListener("click", () => {
  const template = document.getElementById("personalDetailsSummaryTemplate");
  const clone = template.content.cloneNode(true);
  document.getElementById("summaryContainer-out").innerHTML = ""; // Clear old summary
  document.getElementById("summaryContainer-out").appendChild(clone);
  populatePersonalSummary();
});
document.getElementById("nextBtn").addEventListener("click", () => {
  const template = document.getElementById("personalDetailsSummaryTemplate");
  const clone = template.content.cloneNode(true);
  document.getElementById("summaryContainer-outer").innerHTML = ""; // Clear old summary
  document.getElementById("summaryContainer-outer").appendChild(clone);
  populatePersonalSummary();
});
//upload ctos images
// work info 
...
document.addEventListener("DOMContentLoaded", () => {
  function updateGradientLines() {
    document.querySelectorAll(".summary-details-out").forEach(form => {
      const gradient = form.querySelector(".gradient-line-side-outer");
      if (!gradient) return;
